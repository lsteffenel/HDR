...
\begin{resume}
	...
	CloudFIT est un intergiciel pour le calcul distribué reposant sur un overlay P2P. D'un point de vue structurel, la spécification de CloudFIT a été faite pour reposer sur les services d'un overlay P2P déjà existant plutôt que de faire toute la gestion des n{\oe}uds et de leur communication (ce qui était le cas avec son prédécesseur CONFIIT \cite{FKF03,Flauzac10}). Ceci a permis une indépendance vis-à-vis des bibliothèques existantes et garantit des propriétés de modularité et maintenance qui ont fait ses preuves au fil des années.
	
	Un autre facteur important du succès de CloudFIT est qu'il supporte autant le paradigme FIIT (Finite number of Independent and Irregular Tasks), que des variantes où les tâches présentent une certaine dépendance (DAGs, Map-Reduce, etc.). Cela signifie que CloudFIT peut être utilisé comme plate-forme d'exécution distribuée pour la plupart des applications capables de répartir le travail en un nombre fini de tâches. 
	
	Au fil des années cette plate-forme a été enrichie avec d'éléments tels que des ordonnanceurs sensibles au contexte ou des mécanismes de stockage DHT. La possibilité d'intervenir sur la totalité de la pile logicielle rend CloudFIT spécialement attractif en tant que plate-forme expérimentale pour le prototypage,  et le test de techniques innovatrices. En effet, c'est un projet que je développe presque entièrement, de la spécification à l'implémentation des fonctionnalités. Grâce à CloudFIT, je peux orienter ma recherche en ce moment sur le \textit{fog computing} et l'Internet des Objets, deux thématiques que je tiens à c{oe}ur. 
	
	Dans ce chapitre je présente d'abord les éléments principaux de l'architecture de CloudFIT et les mécanismes liés à la gestion des n{oe}uds et des tâches. Par la suite, on regardera en plus de détails les stratégies pour l'ordonnancement adapté au contexte et aussi les techniques que nous avons mis en place pour la structuration multi-échelle du réseau ou bien de pour augmenter la performance des applications \textit{big data} grâce au renforcement de la \textit{data-locality}. 
